...
describe('UsersPage', () => {
  ...
  it('invite rotate shows new code in dialog', async () => {
    ...
    const dialog = await screen.findByRole('dialog', { name: /yeni davet kodu/i })
    expect(dialog).toHaveAccessibleDescription(/eski davet kodu artık geçersizdir/i)
    expect(screen.getByText(/NEW-CODE-123/i)).toBeInTheDocument()
  })
})
